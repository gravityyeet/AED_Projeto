...
Sala *analisar_salas (LabList *lista, int l, int c, int fase_proj) {
    Parede *p_node = NULL;
    Sala *head = NULL, *node = NULL;
    int Sala_d = 0;

    p_node = A6_salas(lista, l, c, &Sala_d, fase_proj);
...
void ver_paredes_sala (Sala *head, LabList *lista, int fase_proj) {
    Parede *p;
    Sala *node = NULL;
    Parede *p_node = NULL;
    ...
    while (p != NULL) {
        ...
        if (tabela[p->l + 1][p->c] == 0) {
            p_node = A6_salas(lista, p->l + 1, p->c, &Sala_d, fase_proj);
            node = criar_sala(p->l + 1, p->c, p_node, Sala_d);
            head = inserir_lista_sala(head, node);
            ver_paredes_sala(node, lista, fase_proj);
        }
        Sala_d = 0;
        if (tabela[p->l - 1][p->c] == 0) {
            p_node = A6_salas(lista, p->l - 1, p->c, &Sala_d, fase_proj);
        ...
        if (tabela[p->l][p->c + 1] == 0) {
            p_node = A6_salas(lista, p->l, p->c + 1, &Sala_d, fase_proj);
...
Parede *A6_salas (LabList *lista, int teste_l, int teste_c, int *sala_d, int fase_proj) {
